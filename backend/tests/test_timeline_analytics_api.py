...
def test_get_analytics_timeline(client: TestClient, db_session: Session) -> None:
    ...
    assert payload["rows"][0]["income"] == "1000.00"
    assert payload["rows"][0]["expenses"] == "100.00"
    assert payload["rows"][0]["savings"] == "50.00"
    assert payload["rows"][0]["net_cash_flow"] == "850.00"
    assert payload["rows"][1]["transfers"] == "200.00"
